function [rect_position,time] = tracker_HC_HCFT(s_frame, pos, target_sz, ...
    padding, lambda, output_sigma_factor, interp_factor,config)
...
time      = 0;
positions = zeros(numel(s_frame), 2);
rect_position = zeros(numel(s_frame), 4);
...
% Start tracking
% ================================================================================
for frame = 1:numel(s_frame),
    im = imread(s_frame{frame}); % Load the image at the current frame
    org_im = im;
    if ismatrix(im)
        im = cat(3, im, im, im);
    end
        ...
        if  max_response< config.motion_thresh
            ...
            if (nonzeros(ind))
              ...
              %patch2 = patch1(bb(2):1:bb(2)+bb(4),bb(1):1:(bb(1)+bb(3)),:);                
                    ...
                    % which can be extended to APEC
                    % 高置信度尺度评估 section 3.5.1
